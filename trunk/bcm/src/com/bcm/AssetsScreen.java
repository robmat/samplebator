...
public class AssetsScreen extends CommonsForScreen implements IWaitableScreen {
	...
	public AssetsScreen(boolean _useCache) {
		useCache = _useCache;
	}

	public AssetsScreen(final String id, final String name, boolean _useCache) {
		useCache = _useCache;
		this.id = id;
		...
		addMenuItem(new MenuItem(I18n.bundle.getString(BcmResource.refreshLbl), 0, 1) {
			public void run() {
				...
				UiApplication.getUiApplication().pushScreen(new AssetsScreen(id, name, false));
			}
		});
		init();
	}
	...
	private void init() {
		Hashtable[] itemsTemp = null;
		if (id == null) {
			itemsTemp = CacheManager.getCache(CacheManager.GET_ALL_ASSETS, null, null);
		} else {
			itemsTemp = CacheManager.getAssetsByProcessIdCache(id, null);
		}
