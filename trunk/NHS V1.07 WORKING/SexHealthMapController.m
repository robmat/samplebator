...
@implementation SexHealthMapController
...
- (void)viewDidLoad {
	...
	double latitude = mapa.userLocation.coordinate.latitude ;
    double longitude  = mapa.userLocation.coordinate.longitude ;
	
	//latitude = 53.713570;
	//longitude = -0.340825;
	
	InWhichCountyAmI* iwcai = [[InWhichCountyAmI alloc] init];
	NSString* countyId = [iwcai giveCountyWithLongtitude:longitude latitude:latitude];
	...
	SexLocations = [dh getDataByCategory:@"sex"]; //[[NSArray alloc] initWithContentsOfFile:[[NSBundle mainBundle] 
																  //pathForResource:@"sex" 
																  //ofType:@"plist"]];
	[SexLocations retain];	
	for (NSDictionary* SexDict in SexLocations){
...
- (MKAnnotationView *)mapView:(MKMapView *)mapView viewForAnnotation:(id <MKAnnotation>)annotation{
	...
	if (annotationView == nil){
		annotationView = [[[MKAnnotationView alloc] initWithAnnotation:annotation 
						  ...
						  autorelease];
		annotationView.image = [UIImage imageNamed:@"4yp_icon3.png"];
...
- (void)mapView:(MKMapView *)mapView annotationView:(MKAnnotationView *)view calloutAccessoryControlTapped:(UIControl *)control
{
	...
	int numElementos = [SexLocations count] ; 
	...
	//[array replaceObjectAtIndex:9 withObject:[dict objectForKey:@"times"]];	
	...
	SexHealthMapDetailsView * sexHealthMapDetailsView = [[SexHealthMapDetailsView alloc] initWithNibName:@"SexHealthDetailsView" bundle:nil ] ;
	[[self navigationController] pushViewController:sexHealthMapDetailsView animated:YES];
	[sexHealthMapDetailsView release] ;
}
...
-(void)viewWillAppear:(BOOL)animated{
	self.title = @"NHS Yorkshire and Humber" ;
	estado_siguiente = -1 ;
