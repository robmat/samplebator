...
@implementation editAllergiesViewController
...
-(IBAction)saveAction:(id)sender{
	...
	[array replaceObjectAtIndex:9 withObject: allergiesField.text ] ;
...
- (BOOL)textFieldShouldReturn:(UITextField *)theTextField{
	...
	NSLog(@"Nuevo allergiesField guardado") ;
...
- (void)viewDidLoad {
	...
	if ( [[NSFileManager defaultManager] fileExistsAtPath:filePath]) {
		NSMutableArray * array = [[NSMutableArray alloc] initWithContentsOfFile:filePath] ;
		allergiesField.text = [array objectAtIndex:9] ;
		NSLog(@"%@", array) ;
		[array release] ;
	}	
	else{
	...
	if ([allergiesField.text isEqualToString:@"Your allergies"]) {
		allergiesField.text = @"" ;
		//allergiesField.placeholder = @"Your allergies" ;
	}
...
-(void)viewDidAppear:(BOOL)animated{
	...
	[allergiesField becomeFirstResponder] ;
}
