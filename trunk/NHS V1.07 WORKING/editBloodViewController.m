...
@implementation editBloodViewController
...
-(IBAction)saveAction:(id)sender{
	...
	[array replaceObjectAtIndex:2 withObject: typeBloodField.text ] ;
...
-(NSInteger) pickerView:(UIPickerView *)thePickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component{
	...
	return [listBlood objectAtIndex:row] ;
...
-(void) pickerView:(UIPickerView *)thePickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component{
	...
	NSLog(@"Selected item: %@. Index of selected item: %i", [listBlood objectAtIndex:row] , row);
...
- (void)viewDidLoad {
	...
	listBlood = [[NSMutableArray alloc] init] ;

	[listBlood addObject:@"Group A" ] ;
	[listBlood addObject:@"Group B" ] ;
	[listBlood addObject:@"Group AB" ] ;
	[listBlood addObject:@"Group 0" ] ;
	[listBlood addObject:@"I'm not sure" ] ;
	...
	if ( [[NSFileManager defaultManager] fileExistsAtPath:filePath]) {
		...
		typeBloodField.text = [array objectAtIndex:2] ;
