...
#import "rac_ipAppDelegate.h"
#import "SplashViewController.h"
#import "VoucherViewcontroller.h"

@implementation rac_ipAppDelegate
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {    
	...
	if (notif) {
		...
		UIAlertView *alert = [[UIAlertView alloc] initWithTitle:title message: body delegate:self cancelButtonTitle:@"OK" otherButtonTitles:  @"Show", nil];
		[alert show];
		[alert release];
...
- (void)application:(UIApplication *)app didReceiveLocalNotification:(UILocalNotification *)notif {
	...
	[alert show];
	[alert release];
	[UIApplication sharedApplication].applicationIconBadgeNumber = notif.applicationIconBadgeNumber - 1;
}
- (void) alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
	if (buttonIndex == 1) {
		VoucherViewcontroller* vvc = [[VoucherViewcontroller alloc] initWithNibName:nil bundle:nil];
		[navController pushViewController:vvc animated:YES];
		[vvc release];
	}
}
